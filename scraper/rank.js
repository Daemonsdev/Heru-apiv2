...
exports.config = {
    ...
    category: 'canvas',
    link: ['/rank?level=102&rank=563&xp=71032&requiredXP=95195&nickname=Jaymar&status=online&avatar=https://imgur.com/xwCoQ5H.jpg']
};
...
exports.initialize = async function ({ req, res }) {
    try {
        ...
        if (!level || !rank || !xp || !requiredXP || !nickname || !status || !avatar) {
            return res.status(400).json({ error: "usage: /rank?level=your_level&rank=your_rank&xp=your_xp&requiredXP=your_required_xp&nickname=your_nickname&status=your_status&avatar=your_avatar_url" });
        }
